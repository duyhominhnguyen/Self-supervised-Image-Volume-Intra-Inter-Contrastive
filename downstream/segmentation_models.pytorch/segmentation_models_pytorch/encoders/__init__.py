...
def get_encoder(name, in_channels=3, depth=5, weights=None, output_stride=32, **kwargs):
    ...
    if weights is not None:
        ...
        try:
            weights = torch.load(settings["url"])
            trunk_weights = weights["state_dict"]
            prefix = "module.intra."
            trunk_weights = {k[len(prefix):] : w for k, w in trunk_weights.items()}
            encoder.load_state_dict(trunk_weights, strict=False)
        except:
            raise Exception("Wrong checkpoint format")
