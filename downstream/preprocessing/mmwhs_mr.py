...
if __name__ == '__main__':
    ...
    dest_dir = '../train/' # dir for saving train images
    dest_dir_label = '../train_labels/'
    ...
    for vol_name in img_fold_list:
        ...
        for depth in range(0, img_flair.shape[2]):
            imsave(os.path.join(dest_dir, vol_name.split('.')[0] + '_frame_' + str(depth) + '.png'), img_flair[:, :, depth], check_contrast=False)
            imsave(os.path.join(dest_dir_label, vol_name.replace('image','label').split('.')[0] + '_frame_' + str(depth) + '.png'), mask[:, :, depth], check_contrast=False)

    ### Validation set
    data_dir = '../Raw/valid/'
    img_fold_list = os.listdir(data_dir)
    dest_dir = '../valid/'
    dest_dir_label = '../valid_labels/'
    if not os.path.exists(dest_dir):
        os.makedirs(dest_dir)
    if not os.path.exists(dest_dir_label):
        os.makedirs(dest_dir_label)
