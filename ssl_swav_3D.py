...
logger = getLogger()
parser = argparse.ArgumentParser(description="Implementation of 3D-SwAV-Universal")
...
parser.add_argument("--d_model", type=int, default = 640,
                    help="output of Transformer model")
#########################
## swav specific params #
#########################
parser.add_argument("--crops_for_assign", type=list, nargs="+", default=[0, 1],
...
parser.add_argument("--nmb_prototypes", default=[3000], type=list,
...
def main():
    ...
    print("Building data done with {} images loaded and {} data loader.".format(len(concat_dataset), len(train_loader)))
    # build model
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue):
    ...
    for it, data in enumerate(train_loader):
        ...
        with torch.no_grad():
            w = model.module.prototypes.prototypes0.weight.data.clone()
            w = nn.functional.normalize(w, dim=1, p=2)
            model.module.prototypes.prototypes0.weight.copy_(w)
                ...
                if queue is not None:
                    if use_the_queue or not torch.all(queue[i, -1, :] == 0):
                        ...
                        out = torch.cat((torch.mm(
                            queue[i],
                             model.module.prototypes.prototypes0.weight.t()
                        ), out))
                    # fill the queue
